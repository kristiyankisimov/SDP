...
bool prim(const Graph& g, vector<Link>& result) {
    set<int> nodes;
    ...
    if (nodeCount > 0) {
        ...
        while (addedNewLink) {
            ...
            for (set<int>::iterator it = nodes.begin();
                    it != nodes.end(); ++it) {

                vector<Edge> adjNodes = g.adj(*it);
                for (int i = 0; i < adjNodes.size(); i++) {
                    if (nodes.find(adjNodes[i].end) == nodes.end()) {
                        Link t;
                        t.start = *it;
            ...
            if (temp.size() != 0) {
                int minIndex = 0;
                int minWeinght = temp[0].weight;

                for (int i = 1; i < temp.size(); i++) {
                    if (temp[i].weight < minWeinght) {
                        minIndex = i;
                        minWeinght = temp[i].weight;
                    }
                }
